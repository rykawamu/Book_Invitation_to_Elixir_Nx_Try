...
引数には、以下の値を渡せます。
 ...
 - テンソルがスカラー（ゼロ次元）であることを意味する数値
 - テンソルがベクトルであることを意味するスカラーの集合したリスト
 - n次元のテンソルを意味する、n個のリスト

それぞれの例をいかに示します。

//list[nx_tensor_01][スカラーのテンソルを作成する]{
  iex> Nx.tensor(0)
  #Nx.Tensor<
    s64
    0
...
//list[nx_tensor_02][ベクトルのテンソルを作る]{
  iex> Nx.tensor([1, 2, 3])
  #Nx.Tensor<
    s64[3]
    [1, 2, 3]
